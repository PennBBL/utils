...
sed -i s@"set fmri(npts) 80"@"set fmri(npts) $dim4"@g $scriptdir/"$unique_id"_design.fsf
...
###this makes wm99 for registration
if [ ! -e $t1dir/wm_seg_thr99.nii.gz ];then
	pve2=`ls $t1dir/*_bias_v2_pve_2.nii.gz`
	logrun fslmaths $pve2 -thr 0.99 $t1dir/wm_seg_thr99.nii.gz
fi

###this performs registration with BBR
if [ ! -e $coreg_outpath/ep2struct.nii.gz ];then
	epiimage=`ls -d $feat_outpath.feat/example_func.nii.gz` # point to example func
	t1head=`ls -d $t1dir/*corrected.nii.gz`
	t1brain=`ls -d $t1dir/*correctedbrain.nii.gz`
	wm=$(ls -d $t1dir/wm_seg_thr99.nii.gz) 
	outname=ep2struct
	wd=$coreg_outpath
	mkdir -p $wd
	fmap=$(ls -d $dir/../B0_map_new/*_mag1.nii)
	fmap_bet=$(ls -d $dir/../B0_map_new/*mag1_t1bet.nii*)
	radmap=$(ls -d $dir/../B0_map_new/*rpsmap.nii)
	echospace=.000568
	wrapdir=-y
	logrun /import/speedy/eons/progs/registration/epi_reg_tds_v3.sh --epi=$epiimage --t1=$t1head --t1brain==$t1brain --t1wm=$wm --out=$outname --wd=$wd  --fmap=$radmap --fmapmag=$fmap --fmapmagbrain=$fmap_bet -v --pedir=$wrapdir --echospacing=$echospace
fi
...
if [ $registration == "ants" ];then
	...
	echo "antsApplyTransforms -d 3 -i $quant_out -o $regpath/$quant_outname.nii.gz -r $template -t $affine -t $rigid -t $warp -t $coreg"
