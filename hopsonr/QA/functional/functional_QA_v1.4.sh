...
while getopts ":a:p:f:s:ho:m:e:w:tx:c:b:v:" OPT
	...
	case $OPT in
		...
		spikerate_cutoff=`echo $OPTARG | cut -d "," -f 4`
		nclips_cutoff=`echo $OPTARG | cut -d "," -f 5`
		;;
		t) #testing mode
		testing=1
		;;
...
if [ -e "$epi10_file" ]; then
	...
	do
		...
		[ $column_name == 'nclip' ] && nclips_field=$field
		field=$((field+1))
	done
	###check that needed fields found
...
header_line=`echo user,hostname,version,no_data_collected,missing_processed_data_exclude,meanrel,maxrel,tsnr,spikerate,nclips,zeros_in_mask,voxels_out_of_mask,fslcc,$measure_head`
for column in `echo $header_line | tr "," " "`;do
new_header_line=`echo $new_header_line,$sequence_name"_"$column`
done
new_header_line=`echo "bblid,scanid"$new_header_line`
...
do
	...
	spikerate=`echo $qa_epi10 | cut -d "," -f $spikerate_field`
	nclips=`echo $qa_epi10 | cut -d "," -f $nclips_field`
	...
	if [ "X$data" == "X" ];then
		echo $bblid,$scanid,$USER,$HOSTNAME,$0,0,1,$meanrel,$maxrel,$meantsnr,$spikerate,$nclips,,,,$measure_output
		subject_errors=`echo $subject_errors,"/import/monstrum/$protocol/subjects/$bblid"_"$scanid/$file_path not found"`
		echo $bblid,$scanid,$USER,$HOSTNAME,$0,0,1,$meanrel,$maxrel,$meantsnr,$spikerate,$nclips,,,,$measure_output >> $out_path
	elif [ `echo $data | wc -w` -gt 1 ];then
		echo "Error - more than one sequence file found on monstrum for "$bblid
...
###remember, $coverage contains TWO values
	echo $bblid,$scanid,$USER,$HOSTNAME,$0,0,0,$meanrel,$maxrel,$meantsnr,$spikerate,$nclips,$coverage,$registration,$measure_output
	echo $bblid,$scanid,$USER,$HOSTNAME,$0,0,0,$meanrel,$maxrel,$meantsnr,$spikerate,$nclips,$coverage,$registration,$measure_output >> $out_path
...
echo /import/monstrum2/Applications/R3.1.2/bin/R --file=/import/speedy/scripts/hopsonr/QA/functional/functional_QA_v1.2.R --slave --args "$out_path" "$r_out_path" "$visual_out_path" "$sequence_name" "$redcap_name" "$meanrelrms_cutoff" "$maxrelrms_cutoff" "$tsnr_cutoff" "$spikerate_cutoff" "$nclips_cutoff"
