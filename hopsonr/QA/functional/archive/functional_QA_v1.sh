...
Usage:
  ...
  -c:  name of easy Condition, as it appears in scoring file. Required.
...
while getopts ":a:p:f:s:ho:m:e:w:tx:c:" OPT
	do
	case $OPT in
		h) #help
		Usage >&2
...
run_qa_epi10 () {
	day=`date +%m_%d_%y`
	lastrun=`basename $out_dir/qa_epi10_*.csv 2> /dev/null | cut -d "." -f 1 | cut -d "_" -f 3,4,5`
	echo "today is: "$day
	...
	if [ $lastrun != $day ];then
		...
		old_test=`ls $out_dir/qa_epi10_*.csv`
...
[ $overwrite == 1 ] && echo bblid,scanid,no_data_collected,missing_data,meanrel,maxrel,tsnr,zeros_in_mask,voxels_out_of_mask,fslcc,all_trials_count,all_trials_correct,easy_condition_count,easy_condition_correct > $out_path
echo bblid,scanid,no_data_collected,missing_data,meanrel,maxrel,tsnr,zeros_in_mask,voxels_out_of_mask,fslcc,all_trials_count,all_trials_correct,easy_condition_count,easy_condition_correct
...
echo $bblid,$scanid,0,0,$meanrel,$maxrel,$meantsnr,$coverage,$registration,$all_count,$all_correct,$easy_count,$easy_correct
echo $bblid,$scanid,0,0,$meanrel,$maxrel,$meantsnr,$coverage,$registration,$all_count,$all_correct,$easy_count,$easy_correct >> $out_path
