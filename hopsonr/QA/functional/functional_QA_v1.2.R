...
sequence_name<-commandArgs(trailingOnly=TRUE)[4]
redcap_project<-commandArgs(trailingOnly=TRUE)[5]
meanrelrms_cutoff<-commandArgs(trailingOnly=TRUE)[6]
maxrelrms_cutoff<-commandArgs(trailingOnly=TRUE)[7]
tsnr_cutoff<-commandArgs(trailingOnly=TRUE)[8]
spikerate_cutoff<-commandArgs(trailingOnly=TRUE)[9]
nclips_cutoff<-commandArgs(trailingOnly=TRUE)[10]
...
if (exists("testing")){
      ...
      qa_path<-"/import/monstrum/eons3_xnat/qa_test/frac2back/frac2back_qa_test.csv"
      out_path<-"/import/monstrum/eons3_xnat/qa_test/frac2back/qa_data_R.csv"
      visual_qa<-"/import/monstrum/eons3_xnat/qa_test/frac2back/qa_data_visual.csv"
      sequence_name<-"frac2back"
...
if(is.na(spikerate_cutoff)){spikerate_cutoff<-20}
if(is.na(nclips_cutoff)){nclips_cutoff<-10}

###test if arguments are valid
if (! file.exists(qa_path)){stop(paste(qa_path," not found.",sep=""))}else{qa_data<-read.csv(qa_path)}

###remove sequence name so everything doesn't need rewritten. Add back in later.
colnames(qa_data)<-gsub(paste(sequence_name,"_",sep=""),"",colnames(qa_data))
...
for (measure in colnames(qa_data)[grepl("_behave",colnames(qa_data))]){
      ...
      print(ggplot(data=qa_data,aes(x=qa_data$scanid,y=qa_data[[measure]],label=qa_data$scanid))+
...
#plot tsnr, meanrelrms, and tsnr v. meanrelrms, spikes, nclips
ggplot(data=qa_data,aes(x=qa_data$scanid,y=qa_data$meanrel,label=qa_data$scanid))+
      geom_hline(y=meanrelrms_cutoff,colour="red")+ylab("Mean Relative RMS")+xlab("Scanid")+
...
ggplot(data=qa_data,aes(x=qa_data$scanid,y=qa_data$tsnr,label=qa_data$scanid))+
      geom_hline(y=maxrelrms_cutoff,colour="red")+ylab("tSNR")+xlab("Scanid")+
      ggtitle(paste("tSNR by Scanid"))+
      theme(axis.title.x = element_text(face="bold", size=20),
            axis.title.y  = element_text(face="bold", size=20),
            axis.text.x=element_text(size=16),axis.text.y=element_text(size=16),
            plot.title=element_text(size=24,face="bold"))+geom_text()
ggplot(data=qa_data,aes(x=qa_data$scanid,y=qa_data$spikerate,label=qa_data$scanid))+
      geom_hline(y=spikerate_cutoff,colour="red")+ylab("Spike Rate")+xlab("Scanid")+
      ggtitle(paste("Spike Rate by Scanid"))+
      theme(axis.title.x = element_text(face="bold", size=20),
            axis.title.y  = element_text(face="bold", size=20),
            axis.text.x=element_text(size=16),axis.text.y=element_text(size=16),
            plot.title=element_text(size=24,face="bold"))+geom_text()
ggplot(data=qa_data,aes(x=qa_data$scanid,y=qa_data$nclips,label=qa_data$scanid))+
      geom_hline(y=nclips_cutoff,colour="red")+ylab("nClips")+xlab("Scanid")+
      ggtitle(paste("nClips by Scanid"))+
      theme(axis.title.x = element_text(face="bold", size=20),
            axis.title.y  = element_text(face="bold", size=20),
            axis.text.x=element_text(size=16),axis.text.y=element_text(size=16),
            plot.title=element_text(size=24,face="bold"))+geom_text()
ggplot(data=qa_data,aes(x=qa_data$scanid,y=qa_data$tsnr,label=qa_data$scanid))+
      geom_hline(y=tsnr_cutoff,colour="red")+ylab("tSNR")+xlab("Scanid")+
      ggtitle(paste("tSNR by Scanid"))+
      theme(axis.title.x = element_text(face="bold", size=20),
            axis.title.y  = element_text(face="bold", size=20),
            axis.text.x=element_text(size=16),axis.text.y=element_text(size=16),
...
ggplot(data=qa_data,aes(x=qa_data$meanrel,y=qa_data$tsnr))+geom_point()+
      geom_hline(y=tsnr_cutoff,colour="red")+geom_vline(x=meanrelrms_cutoff,colour="red")+ylab("tSNR")+xlab("Mean Relative RMS")+
...
####rename columns w/ sequence as prefix for redcap integration
ncolumns<-ncol(qa_data)
colnames(qa_data)[3:ncolumns]<-paste(sequence_name,colnames(qa_data)[3:ncolumns],sep="_")
