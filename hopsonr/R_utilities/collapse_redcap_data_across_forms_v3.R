...
if (exists("testing")){
  ...
  range=2000
  out_path<-"/import/monstrum/Users/hopsonr/test_collapsed_output.csv"
}
#####
...
for (i in 1:nrow(ids)){
  ...
  #make temp of bblid
  temp<-project_data[which(project_data$bblid == bblid & project_data$dovisit2 > date - range & 
                            project_data$dovisit2 < date + range),]
  ...
  for (measure in measures){
    ...
    ###first measure creates a new row, each additional fills in the cells associated with that measure in the data dictionary
    if (! paste(bblid,date) %in% paste(output$bblid,output$date_provided)){
      row$date_provided<-date
      output<-rbind(output,row)
      }else{
      output[which(output$bblid==bblid & output$date_provided==date),c(project_dictionary$field_name[which(project_dictionary$form_name==measure)])]<-row[,c(project_dictionary$field_name[which(project_dictionary$form_name==measure)])]
    }
    output[which(output$bblid==bblid & output$date_provided==date),paste(measure,"distance",sep="_")]<-row$dist
    #print(ncol(output))
  }
  ...
  ###this next bit is nuts. all of the general fields exist for all measures, so for each of those fields I get all the values from the day
  ###closest to the day specified, then if there are any values ≠ "" or NA, it assigns that field the most common one
  #for (general_field in project_dictionary$field_name[which(project_dictionary$form_name=="general")]){
  #  if (0 < length(temp[which(abs(temp$dist) == min(abs(temp$dist[which(temp$procedure==measure)])) & ! is.na(temp[[general_field]]) & ! temp[[general_field]]== ""),general_field])){
  #    output[which(output$bblid == bblid),general_field]<-names(sort(table(temp[which(abs(temp$dist) == min(abs(temp$dist[which(temp$procedure==measure)])) & ! is.na(temp[[general_field]]) & ! temp[[general_field]]== ""),general_field]),decreasing=T)[1])
  #  }
  #}
}
