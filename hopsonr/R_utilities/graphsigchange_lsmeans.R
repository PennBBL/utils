...
graphsigchange <- function(data=NULL, roi_names=NULL,value=NULL,x.value=NULL,group.value=NULL,scale.fill=NULL,ranges=NULL,test=FALSE,paired=FALSE,y.title="% Signal Change",x.title="Trial Type",graph.title=NULL,for.presentation=FALSE,x.axis.text.size=24,x.angle=0) {
  ...
  x<-data.frame()
  model1<-lme(fixed,random,data=roi_values)
  anova.lme.out<-anova.lme(model1,type="marginal",adjustSigma=F)
  datasum<-summary(lsmeans(
...
#    if (for.presentation){
#      g<-g+theme(axis.title.x=element_text(size=36))+theme(axis.title.y=element_text(size=36))+theme(axis.text.x=element_text(size=x.axis.text.size,angle=x.angle))+
#      theme(axis.text.y=element_text(size=24))+theme(title=element_text(size=48))+
